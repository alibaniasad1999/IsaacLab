...
% This is the SINGLE source .tex; build it from its own directory with:
%   pdflatex --output-directory=slides_output/slides cable_slides.tex   (run twice)
% Frame images are referenced as slides_output/<approachX>/frame.png, i.e. relative
% to THIS file's directory (scripts/cable_simulation/). run_all_and_capture.sh does
% exactly that. \graphicspath also allows a build from inside slides_output/slides/.
\graphicspath{{./}{../}}
...
\begin{frame}{Approach 1 --- Capsule chain (D6 joints)}
  \begin{columns}[c]
    ...
    \column{0.54\textwidth}
      \simframe{slides_output/approach1_cable_only/frame.png}{Video: approach1\_cable\_only/video.mp4 (shared folder)}
  \end{columns}
\end{frame}

% ---------- 5. Approach 1 -- robot ----------
\begin{frame}{Approach 1 --- With robot}
  \simframe{slides_output/approach1_robot/frame.png}{Video: approach1\_robot/video.mp4 (shared folder)}
  \begin{center}\footnotesize
...
\begin{frame}{Approach 2 --- Cosserat rod in NVIDIA Warp}
  \begin{columns}[c]
    \column{0.46\textwidth}
      ...
      \end{itemize}
    \column{0.54\textwidth}
      \simframe{slides_output/approach2_cable_only/frame.png}{Video: approach2\_cable\_only/video.mp4 (shared folder)}
  \end{columns}
\end{frame}

% ---------- 7. Approach 2 -- robot ----------
\begin{frame}{Approach 2 --- With robot}
  \simframe{slides_output/approach2_robot/frame.png}{Video: approach2\_robot/video.mp4 (shared folder)}
  \begin{center}\footnotesize
...
\begin{frame}{Approach 3 --- FEM deformable (Isaac Sim)}
  \begin{columns}[c]
    \column{0.46\textwidth}
      ...
      \end{itemize}
    \column{0.54\textwidth}
      \simframe{slides_output/approach3_cable_only/frame.png}{Video: approach3\_cable\_only/video.mp4 (shared folder)}
  \end{columns}
\end{frame}

% ---------- 9. Approach 3 -- robot ----------
\begin{frame}{Approach 3 --- With robot}
  \simframe{slides_output/approach3_robot/frame.png}{Video: approach3\_robot/video.mp4 (shared folder)}
  \begin{center}\footnotesize
