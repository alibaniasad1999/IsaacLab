...
from pxr import UsdPhysics, Gf, Sdf, UsdGeom, UsdLux
...
# -------------------------------------------------------------
# Lighting rig: load the "Grey Studio" preset that the GUI
# (Stage > Lighting > Lighting Rigs) provides.
# -------------------------------------------------------------
def load_lighting_rig(rig_name: str = "Grey_Studio"):
    """Reference an Isaac Sim built-in lighting-rig USD into the stage."""
    matches = []
    for sp in site.getsitepackages() + [site.getusersitepackages()]:
        matches.extend(
            glob.glob(
                f"{sp}/isaacsim/extscache/omni.kit.viewport.menubar.lighting-*/"
                f"data/usd/{rig_name}.usda"
            )
        )

    if not matches:
        print(f"Lighting rig '{rig_name}' not found — skipping.")
        return

    rig_usd = matches[0]
    print(f"Loading lighting rig: {rig_usd}")

    # Remove the default distant light that add_default_ground_plane creates,
    # so the rig's lights are what we see.
    for path in ("/World/defaultGroundPlane/SphereLight", "/World/defaultGroundPlane/DistantLight"):
        prim = stage.GetPrimAtPath(path)
        if prim and prim.IsValid():
            stage.RemovePrim(path)

    rig_prim = UsdGeom.Xform.Define(stage, "/World/LightingRig")
    rig_prim.GetPrim().GetReferences().AddReference(rig_usd)


load_lighting_rig("Grey_Studio")
